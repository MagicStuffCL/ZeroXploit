			...
			function jbdone() {			
				sleep(2000).then(() => {
					...
					else if(requireGoldhen == 2){
						showMessage("Jailbreak Ready");
					}
				});
			}

			function allset() {
				showMessageColor (LoadedMSG,"green");
			}

			function Binset() {
			showMessage ("Payload Loaded. Send payloads to port 9020");
			alert("Payload Loaded. Send payloads to port 9020");
			}

			function Mset() {
			showMessage ("Payload Loaded. Send payloads to port 9021");
			...
			$(function(){				
				...
				$.ajax({
					url: 'TrainersUpdated.json',
					dataType : 'json',
					type:  'get',
					success:  function (response) {			
						$("#trainersDate").html(response.TrainersUpdated);
					},
					error: function(){ // will fire when timeout is reached
						$("#trainersDate").html("ERROR");
					},
					timeout: 5000 // sets timeout to 5 seconds
				});						
			...
			function Bloader(){
				showMessageColor("Payload loading... please wait","red");
